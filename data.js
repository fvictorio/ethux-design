...
const DATA = {
  categories: [
    { id:'getting-started', title:'User Onboarding', icon:'rocket', img:'images/onboard.svg', color:'var(--cat-1)', hook:'Make the first 5 minutes effortless.', desc:'The moment someone decides to try Ethereum, a clock starts. Within five minutes, they will either complete their first action or leave permanently. These are the walls between curiosity and that first transaction.', problems:[
      { title:'The Gas Hurdle', severity:'critical', status:'in-progress', desc:'Users must buy and transfer ETH to their wallet before they can do almost anything. This multi-step onramp process creates a massive barrier for newcomers who just want to try their first transaction. Sponsoring initial transactions through paymasters is the most promising path forward.', solutions:[{name:'Paymasters (ERC-4337)',status:'Live',adoption:'54M+ smart accounts (bundlebear.com)'},{name:'EIP-7702 Delegation',status:'Live',adoption:'9 wallets on mainnet (7702beat)'},{name:'SmolRefuel',status:'Live',adoption:'80+ chains (DefiLlama founder)'},{name:'Embedded wallets with paymaster integration',status:'Live',adoption:'Primary delivery vehicle for sponsored onboarding'}], eips:['ERC-4337','EIP-7702','ERC-3009'], checklist:'gas' },
    ...
    { id:'cross-chain', title:'Cross-chain Flow', icon:'chain', img:'images/crosschain.svg', color:'var(--cat-3)', hook:'Make moving between chains invisible.', desc:'Ethereum now spans dozens of networks. Users see fragmented balances, manual chain switching, and bridges that feel like sending money into a void. The protocol solved scaling. The UX hasn\'t caught up.', problems:[
      { title:'Single-Chain Balance Display', severity:'medium', status:'in-progress', desc:'Users must manually switch networks to see their full balances. Portfolio tracking across chains requires cognitive overhead.', solutions:[{name:'Unified balance display',status:'Live',adoption:'Common in major wallets'},{name:'EIP-7811 unified balances',status:'Draft',adoption:'No implementations yet'},{name:'Chain abstraction',status:'Live',adoption:'Particle Network Universal Accounts live, 90+ teams integrating'}], eips:['EIP-7811'], checklist:'multichain' },
      { title:'Manual Network Switching', severity:'medium', status:'in-progress', desc:'When a wallet is connected to chain A and the user wants chain B, they must manually switch.', solutions:[{name:'Auto-switching apps',status:'Building',adoption:'Few apps. EF-funded interop-sdk by Wonderland (ERC-7930/7828) now available for developers.'}], eips:['ERC-7828','EIP-3326'], checklist:'multichain' },
      { title:'Bridging Pain', severity:'critical', status:'in-progress', desc:'Moving assets between chains requires finding a bridge that supports the specific route, comparing fees and wait times across multiple providers, and then waiting minutes to days without knowing if funds are in transit or lost. For small amounts, bridge fees can exceed the value being transferred. Many chain pairs have no direct bridge at all, forcing users through intermediate chains.<br><br>The entire process feels like sending money into a void with no tracking, no estimated arrival, and no recourse if something goes wrong.', solutions:[{name:'ERC-7683 cross-chain intents',status:'Live',adoption:'$35B+ lifetime via Across, EF + Hyperlane Open Intents Framework'},{name:'Native bridges',status:'Live',adoption:'Per-chain'}], eips:['ERC-7683'], checklist:'multichain' },
    ...
    { id:'safety', title:'Safety & Security', icon:'shield', img:'images/security.svg', color:'var(--cat-4)', hook:'Build trust through transparency and user control.', desc:'One bad experience, one lost transaction, and a user is gone for good. Solutions should empower user agency with transparent, controllable defenses rather than opaque restrictions users never opted into.', problems:[
      { title:'Prevalence of Scams', severity:'critical', status:'in-progress', desc:'Phishing attacks and address poisoning cause thousands of victims monthly. But scams don\'t only happen in transactions. They happen in support channels too.<br><br>Users who get stuck search for help and end up in Telegram groups or Discord servers run by impersonators posing as official support. Legitimate help exists for major wallets, but it is hard to find and easy to fake. Several protocols have stopped using Discord for support entirely because scam messages were reaching users faster than moderators could remove them.', solutions:[{name:'Transaction simulation',status:'Live',adoption:'Standard in major wallets (walletbeat.eth)'},{name:'Address poisoning detection',status:'Building',adoption:'Emerging in some wallets, Blockaid integration'},{name:'Wallet warnings',status:'Live',adoption:'Standard in major wallets'},{name:'Official help centers',status:'Building',adoption:'Major wallets and protocols moving to dedicated portals'}], eips:['ERC-7730'], checklist:'safety' },
      { title:'Key Management Burden', severity:'high', status:'in-progress', desc:'Users are solely responsible for securing a private key or seed phrase that controls all their assets. Lose it and everything is gone. Get phished and everything is gone. There is no recovery, no reset password, no customer support. This single point of failure is the core security tradeoff of self-custody and one of the key reasons many users stay on centralized platforms.', solutions:[{name:'Smart accounts (ERC-4337)',status:'Live',adoption:'54M+ accounts, 1B+ UserOps (bundlebear.com)'},{name:'EIP-7702 delegation',status:'Live',adoption:'9 wallets on mainnet (7702beat)'},{name:'Social recovery',status:'Live',adoption:'Growing, 44% YoY adoption'},{name:'Passkey-based signing',status:'Building',adoption:'Emerging'}], eips:['ERC-4337','EIP-7702','EIP-8141'], checklist:null },
    ...
    { id:'daily-ops', title:'Daily Operations', icon:'grid', img:'images/daily.svg', color:'var(--cat-8)', hook:'Polish the flows people use every day.', desc:'The tasks people do every day: sending tokens, checking balances, managing gas. These should be the most polished flows in the ecosystem. They are often the most frustrating.', problems:[
      { title:'Unpredictable Gas Fees', severity:'medium', status:'in-progress', desc:'Gas costs have dropped dramatically. Mainnet averages around 3 gwei in 2026, and L2 transactions cost fractions of a cent.<br><br>But the UX problem persists in a different form: users still cannot predict what a transaction will cost before they commit, fee spikes during network congestion still catch people off guard, and the mental model of paying a variable fee for every action remains foreign to anyone coming from traditional apps.', solutions:[{name:'EIP-1559 fee market',status:'Live',adoption:'Universal'},{name:'EIP-4844 L2 costs',status:'Live',adoption:'All major L2s'},{name:'Predictive fee UI with timing guidance',status:'Live',adoption:'External tools only (Blocknative, Gas Fee Predictor) — not in wallets'}], eips:['EIP-1559','EIP-4844'], checklist:'gas' },
      { title:'Fiat-Based Values Missing', severity:'medium', status:'in-progress', desc:'Some app interfaces still force users to enter transaction amounts denominated in the token (e.g. 0.0042 ETH) rather than in their local currency. Users think in their native fiat, not in token amounts. Having to mentally convert token prices before every send or swap adds friction and increases the chance of costly input errors.<br><br>Additionally, wallets and centralized exchanges increasingly support local currency display beyond just USD and EUR, including currencies like PLN, CZK, BRL, and others. But app interfaces have not kept up, often showing only token amounts or at best USD equivalents. Users outside the dollar economy are left doing double mental conversion: token to USD, then USD to their currency.', solutions:[{name:'Fiat denomination toggle',status:'Live',adoption:'Common in major wallets'},{name:'Price oracle display',status:'Live',adoption:'Standard practice'}], eips:[], checklist:'gas' },
      { title:'Tax Tracking Lives Outside Wallets', severity:'medium', status:'in-progress', desc:'Capable trackers exist — rotki, CoinTracker, Waltio, Koinly — but every one is a separate app you have to feed your wallets, exchanges, and addresses into. Categorization of complex DeFi positions remains imperfect, cross-chain cost basis is fragile, and free tiers cap at low event counts. No major wallet ships native tax-and-portfolio reporting as a first-class feature. Active DeFi users still spend hours each tax season reconciling output.', solutions:[{name:'Third-party trackers (rotki, CoinTracker, Koinly, Waltio)',status:'Live',adoption:'Separate apps, manual reconciliation required'},{name:'Wallet-native tax tracking',status:'None',adoption:'No major wallet ships this as first-class'}], eips:[], checklist:null },
      { title:'NFTs Don\'t Load', severity:'medium', status:'in-progress', desc:'Wallets struggle to display NFTs across multiple chains. The underlying problem is indexing: each blockchain requires separate infrastructure to read ownership, metadata, and media. RPC endpoints go down, indexers return stale data, and IPFS-hosted media loads slowly or not at all.<br><br>The result is blank thumbnails, missing collections, and users who think their NFTs are gone when they are just on a chain the wallet does not index.', solutions:[{name:'Multi-chain NFT indexing',status:'Building',adoption:'Uneven across wallets'},{name:'NFT metadata standards',status:'Live',adoption:'Widespread'}], eips:['ERC-721','ERC-1155'], checklist:null },
  ...
  checklists: [
    ...
    { id:'multichain', title:'Multi-Chain', desc:'Cross-chain balance aggregation, automatic network switching, bridging UX, and smart account address consistency.', available:true, patterns:5, standards:['ERC-7683','ERC-7930','ERC-7828'], items:[
      ...
      { text:'Auto-switch networks when user interacts with a different chain', eip:'ERC-7828', priority:'high', benefit:'Removes the manual "switch network" step. Users interact with apps without thinking about which chain they are on.' },
    ...
    { id:'wallets', title:'Wallet Connection', desc:'EIP-6963 multi-wallet discovery, mobile deep linking, session persistence, and embedded wallets.', available:true, patterns:5, standards:['EIP-6963','EIP-7702','WalletConnect v2'], items:[
      ...
      { text:'Remember the last connected wallet and auto-reconnect on return visits', eip:null, priority:'high', benefit:'Returning users skip the connect flow entirely. Removes a friction point that makes apps feel stateless compared to traditional apps.' },
...
const PROBLEM_DETAILS = {
  ...
  // Transaction Clarity
  'Signing Fatigue': {
    opportunity: '<div class="lead">Batching signatures into a single approval reduces signing friction to one step.</div><ul><li>Session keys enable "one-click DeFi" experiences</li><li>Fewer popups means higher task completion rates</li><li>Users spend time using apps instead of approving requests</li></ul>',
  ...
  'Missing Signing Context': {
    ...
    risk: '<div class="lead">Users who cannot follow the signing sequence either abandon or approve blindly.</div><ul><li>Mid-flow abandonment wastes gas on completed steps with no result</li><li>Users trained to click through prompts without reading become vulnerable to phishing</li><li>No standard exists for apps to communicate expected signing sequences to wallets</li></ul>'
  },
  ...
  'Manual Network Switching': {
    opportunity: '<div class="lead">Auto-switching removes an entire interaction step from every cross-chain action.</div><ul><li>Apps with transparent switching see higher task completion</li><li>Users never see a "wrong network" error again</li><li>Reduces the cognitive load of multi-chain usage</li></ul>',
  ...
  'Mobile Apps Don\'t Respond': {
    opportunity: '<div class="lead">Reliable mobile SDKs unlock 60%+ of web traffic for apps.</div><ul><li>First-mover wallets with smooth mobile flows capture the majority of new users</li><li>Mobile-first markets represent the highest growth potential</li><li>Smooth mobile UX is the #1 competitive differentiator</li></ul>',
    risk: '<div class="lead">Unresponsive wallets train users to avoid mobile apps entirely.</div><ul><li>The market loses access to the largest computing platform on Earth</li><li>Users who fail on mobile don\'t switch to desktop, they leave</li><li>Mobile avoidance becomes a permanent habit</li></ul>'
  },
  // Language
  'Assumed User Knowledge': {
    opportunity: '<div class="lead">Products that explain as they go retain users that expert-first products lose.</div><ul><li>Inline explanations at the point of action turn confusion into learning</li><li>Simple/advanced mode lets beginners use the product without understanding everything upfront</li><li>Users who learn by doing become power users. Users who are expected to know everything upfront leave.</li></ul>',
    risk: '<div class="lead">Products that assume knowledge filter out everyone except insiders.</div><ul><li>Users who don\'t understand what a transaction does make expensive mistakes</li><li>Each confusing experience gets shared as "crypto is too complicated"</li><li>The ecosystem self-selects for technical users and never reaches mainstream adoption</li></ul>'
  },
  'Poor Localization': {
  ...
  'No Default Native Account Abstraction': {
    ...
    opportunity: '<div class="lead">Native AA would let every Ethereum user benefit from smart accounts without opting in.</div><ul><li>Smart wallets retain 70% of users vs 60% for seed-phrase wallets. Social recovery adoption is growing 44% YoY.</li><li>Gas sponsorship, social recovery, and session keys become universal rather than opt-in</li><li>App developers can assume every user has a smart account, simplifying integration</li><li>Ethereum closes the gap with chains like Starknet where this is already the default</li></ul>',
    risk: '<div class="lead">Without native AA, the ecosystem remains split between EOA and smart account users.</div><ul><li>Apps must support two account models, increasing complexity and fragmentation</li><li>Users on EOAs miss out on safety features like transaction simulation and spending limits</li><li>The UX gap between Ethereum and native-AA chains becomes a competitive disadvantage</li></ul>'
  },
...
const RESOURCE_URLS = {
  ...
  'ERC-7730': { url: 'https://clearsigning.org/', label: 'clearsigning.org' },
  'ENS': { url: 'https://docs.ens.domains/', label: 'ENS Docs' },
  'ERC-7930': { url: 'https://github.com/defi-wonderland/interop-sdk', label: 'interop-sdk (Wonderland)' }
};
...
const SKILL_DESCRIPTIONS = {
  approvals: {
    ...
    useCases: ['Building a swap or DEX interface', 'Any app that moves ERC-20 tokens on behalf of users', 'Implementing gasless approvals with Permit2']
  },
  signing: {
    ...
    useCases: ['Any app that requests wallet signatures', 'Login/authentication flows (SIWE)', 'DeFi interactions requiring multiple signatures']
  },
  gas: {
    ...
    useCases: ['Onboarding flows for new users with no ETH', 'Any transaction-heavy application', 'L2 apps where fee display is confusing']
  },
  multichain: {
    ...
    useCases: ['Portfolio or dashboard interfaces', 'Apps deployed on multiple chains', 'Cross-chain transfer or bridge interfaces']
  },
  onboarding: {
  ...
  wallets: {
    ...
    useCases: ['Connect Wallet buttons and modals', 'Mobile app experiences', 'Apps supporting multiple wallet providers']
  },
  safety: {
